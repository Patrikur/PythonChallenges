...
def check_plagiarism(file1, file2):
    ...
    for line in openfile1:
        ...
        f1words.append(line_split)

    for line in openfile2:
        line_split = line.strip("\n").split()
        f2words.append(line_split)

    word_string = ""
    for wordlist in f1words:
    ...
    for j in range(0, len(f1)):
        ...
        if phrase_1 in f2_comp:
            amount = 1

            while True:
                phrase_1 += " " + f1[j + amount]

                if phrase_1 in f2_comp:
                    amount += 1
                    listed_phrase_1 = phrase_1.split()

                    if len(listed_phrase_1) >= len(longest_similarity):
                        longest_similarity = listed_phrase_1
    ...
    loop = 0
    finals = ""
    for word in longest_similarity:
        loop += 1
        if loop == 1:
            finals += word
        else:
            finals += " " + word
    if len(longest_similarity) >= 5:
        return finals
    else:
        return False
